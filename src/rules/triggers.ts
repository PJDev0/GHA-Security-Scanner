import { SecurityRule, Finding, ParsedWorkflow } from '../core/types';

export const DangerousTriggerRule: SecurityRule = {
  id: 'GHA-002',
  ...
  detect(workflow: ParsedWorkflow): Finding[] {
    const findings: Finding[] = [];
    const on = workflow.document.on;
    
    if (!on) return findings;
...
function analyzeDangerousTrigger(trigger: string, workflow: ParsedWorkflow): Finding[] {
  const findings: Finding[] = [];
...
function checkUntrustedCheckout(document: any): boolean {
  if (!document.jobs) return false;
  ...
  for (const job of Object.values(document.jobs)) {
    ...
    for (const step of j.steps) {
      if (step.uses?.includes('checkout')) {
        ...
        if (checkoutPattern.test(ref)) return true;
      }
      if (step.run && checkoutPattern.test(step.run)) return true;
    }
  }
  return false;
}

function checkWritePermissions(document: any): boolean {
  if (!document.permissions) return true;
  if (document.permissions === 'write-all') return true;
  
  const writePerms = ['contents', 'actions', 'security-events'];
  return writePerms.some(perm => 
    document.permissions[perm] === 'write' || document.permissions[perm] === 'admin'
  );
}